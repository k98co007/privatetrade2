...
| **문서 버전** | 1.1.0 |
| **작성일** | 2026-02-16 |
| **기반 문서** | SRS v1.1.0, 유저스토리 v1.1.0, 사용자 인터뷰 (2026-02-15, 2026-02-16) |
| **티켓** | TICKET-068 |
...
- 2026-02-16 인터뷰 기반 신규 전략 2건(전략 A, 전략 B) 아키텍처 반영
- 전략 모듈 확장 영향 정의: 전략 인터페이스, StrategyRegistry, 일중 실행 흐름(손절 미적용/매수·매도 교대)
- WebAPI 전략 enum/검증 범위 확장(기존 3종 → 5종)
- Frontend 전략 선택 옵션 확장(전략 1/2/3/A/B)
- 모듈 분해 섹션에 명시적 모듈 리스트 및 LLD 타깃(STRATEGY) 표기 추가
...
본 문서는 주식투자 전략 시뮬레이션 시스템의 고수준 설계(High-Level Design)를 정의한다. SRS v1.1.0에서 명세된 17개 기능 요구사항(FR-001~FR-017)과 8개 비기능 요구사항(NFR-001~NFR-008)을 충족하는 시스템 아키텍처, 모듈 구조, 기술 스택, 데이터 모델, 배포 구조를 설계하여 상세 설계(LLD) 및 구현의 기반을 제공한다.
...
| 문서 | 경로 | 설명 |
|------|------|------|
| SRS v1.1.0 | docs/srs/srs-v1.1.0.md | 소프트웨어 요구사항 명세서 (FR 17개, NFR 8개) |
...
| **전략 모듈** | Strategy Pattern (GoF) | 전략 인터페이스 기반으로 5개 전략 구현(기존 3 + 신규 2), 신규 전략 추가 시 기존 코드 무변경 (FR-006, FR-016, FR-017, NFR-008) |
...
| 전략 엔진 모듈 | 전략별 매수/매도 판단 | `evaluate(daily_candles, rsi_data, seed_money)` | **STRATEGY** |
...
| **책임** | Yahoo Finance API를 통한 5분 분봉 데이터 조회 및 RSI 지표 산출 |
...
| 항목 | 내용 |
|------|------|
| **관련 요구사항** | FR-003, FR-004, FR-005, FR-006, FR-016, FR-017, NFR-008 |
| **책임** | 투자 전략 인터페이스 정의 및 5개 전략 구현체 제공. Strategy Pattern 기반 확장 구조 |
...
| 항목 | 내용 |
|------|------|
| **관련 요구사항** | FR-010, FR-011, FR-012, FR-016, FR-017 |
...
| `RequestValidator` | Pydantic 기반 요청 데이터 유효성 검증 (심볼 형식, 전략 enum: 1/2/3/A/B) |
...
| 항목 | 내용 |
|------|------|
| **관련 요구사항** | FR-010, FR-011, FR-012, FR-016, FR-017, NFR-003, NFR-004 |
...
| `StrategySelector` | 전략 1/2/3/A/B 옵션 선택 (라디오 또는 셀렉트) | FR-010, FR-016, FR-017 |
...
sequenceDiagram
    ...
    SIM->>DATA: fetch_market_data(symbol, 60d, 5m)
...
| `strategy` | string | Y | 전략 식별자 | `sell_trailing_stop`, `buy_sell_trailing_stop`, `rsi_buy_sell_trailing_stop`, `rsi_only_trailing_stop`, `buy_trailing_then_sell_trailing` 중 하나 |
...
| ④ 거래일 분리 | DailyCandles | DataFrame | 1일 분봉 (~78행, 09:00~15:30) |
...
**v1.1.0 실행 흐름 영향 (신규 전략 A/B):**

- 기존 전략(1/2/3)은 `15:00 손절(강제 청산)` 분기를 유지한다.
- 신규 전략(A/B)은 당일 손절 분기를 비활성화하고, `이익보전 매도 조건 충족 시점`에만 청산한다.
- 신규 전략(A/B)은 `포지션 보유 중 추가 매수 금지`와 `매수→매도 교대` 규칙을 공통 적용한다.

```mermaid
flowchart TD
    START[거래일 시작] --> GET_CANDLES[해당 일자 5분 분봉 추출<br/>09:00 ~ 15:30]
    ...
    BUY --> MONITOR[이익보전 추적<br/>5분봉 단위 모니터링]
...
| 메서드 | 파라미터 | 반환 타입 | 설명 |
|--------|---------|----------|------|
| `evaluate()` | daily_candles, rsi_data, seed_money | TradeSignal | 거래일 전체 5분 분봉을 순회하며 매수/매도 판단. 템플릿 메서드 패턴으로 should_buy→should_sell→should_stop_loss 순서로 호출 |
| `should_buy()` | candle (현재 5분봉), context | BuySignal | 현재 캔들에서 매수 조건 충족 여부 판단. 전략별로 상이한 매수 로직 구현 |
| `should_sell()` | candle, context | SellSignal | 보유 중 현재 캔들에서 매도(이익보전) 조건 충족 여부 판단 |
| `should_stop_loss()` | candle, context | bool | 15:00 도달 시 손절매 조건 판단 (전략 A/B는 항상 False 반환) |
| `initialize_day()` | opening_candle (09:05 캔들) | void | 거래일 초기화. 기준가 설정 등 전략별 일일 초기 상태 설정 |
...
**영향 범위**: 전략 엔진의 신규 클래스/레지스트리 등록이 핵심이며, 외부 인터페이스 일관성을 위해 WebAPI 전략 enum/validator와 프론트엔드 `StrategySelector` 옵션을 함께 확장한다. 시뮬레이션 엔진·결과 보고서 모듈의 핵심 실행/집계 로직은 변경 없이 재사용된다 (OCP 원칙 준수, FR-006, NFR-008).

### 7.5 전략 확장 모듈 영향 및 인터페이스

| 영향 모듈 | 변경 포인트 | 인터페이스 영향 |
|-----------|------------|----------------|
| 전략 엔진 모듈 | 신규 전략 클래스 2종 추가(전략 A/B), `StrategyRegistry` 등록 항목 확장 | `BaseStrategy.evaluate()` 계약 유지, `should_stop_loss()` 전략별 동작 분기 |
| 웹 API 모듈 | 요청 모델 전략 enum 5종으로 확장, 검증 에러 메시지 갱신 | `POST /api/simulations`의 `strategy` 입력 허용값 확장 |
| 프론트엔드 모듈 | `StrategySelector` 옵션 확장(전략 1/2/3/A/B) | 시작 요청 payload의 `strategy` 값 매핑 테이블 확장 |
...
| `strategy` | TEXT | NOT NULL | 전략 식별자 | FR-003~005, FR-016, FR-017 |
...
| 항목 | 수량 | 커버리지 |
|------|------|---------|
| 기능 요구사항 (FR) | 17 (FR-001 ~ FR-017) | **17/17 (100%)** |
