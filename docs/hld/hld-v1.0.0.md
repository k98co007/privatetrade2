...
| **문서 버전** | 1.0.0 |
| **작성일** | 2026-02-15 |
| **기반 문서** | SRS v1.0.0, 유저스토리 v1.0.0, 사용자 인터뷰 (2026-02-15) |
| **티켓** | TICKET-003 |
...
본 문서는 주식투자 전략 시뮬레이션 시스템의 고수준 설계(High-Level Design)를 정의한다. SRS v1.0.0에서 명세된 15개 기능 요구사항(FR-001~FR-015)과 8개 비기능 요구사항(NFR-001~NFR-008)을 충족하는 시스템 아키텍처, 모듈 구조, 기술 스택, 데이터 모델, 배포 구조를 설계하여 상세 설계(LLD) 및 구현의 기반을 제공한다.
...
| 문서 | 경로 | 설명 |
|------|------|------|
| SRS v1.0.0 | docs/srs/srs-v1.0.0.md | 소프트웨어 요구사항 명세서 (FR 15개, NFR 8개) |
| 유저스토리 v1.0.0 | docs/userstory/userstory-v1.0.0.md | 17개 유저스토리 정의 |
...
| **전략 모듈** | Strategy Pattern (GoF) | 전략 인터페이스 기반으로 3개 전략 구현, 신규 전략 추가 시 기존 코드 무변경 (FR-006, NFR-008) |
...
| **책임** | 투자 전략 인터페이스 정의 및 3개 전략 구현체 제공. Strategy Pattern 기반 확장 구조 |
...
| **책임** | REST API 엔드포인트 제공, SSE 실시간 이벤트 스트리밍, 입력 검증 |
...
| `strategy` | string | Y | 전략 식별자 | `sell_trailing_stop`, `buy_sell_trailing_stop`, `rsi_buy_sell_trailing_stop` 중 하나 |
...
| `should_stop_loss()` | candle, context | bool | 15:00 도달 시 손절매 조건 판단 |
...
4. **프론트엔드 업데이트**: 전략 선택 UI(`StrategySelector`)에 새 전략 옵션을 추가한다.
...
**영향 범위**: 기존 전략 코드, 시뮬레이션 엔진, 결과 보고서 모듈에 대한 수정이 불필요하다. 새 전략 클래스 파일 생성과 레지스트리 등록만으로 완료된다 (OCP 원칙 준수, FR-006, NFR-008).
...
| `strategy` | TEXT | NOT NULL | 전략 식별자 | FR-003~005 |
...
| NFR-008 | 전략 확장 용이성 (OCP) | BaseStrategy ABC + StrategyRegistry, Strategy Pattern, 4단계 추가 절차 | 7.1, 7.4 |

### 10.3 커버리지 요약

| 항목 | 수량 | 커버리지 |
|------|------|---------|
| 기능 요구사항 (FR) | 15 (FR-001 ~ FR-015) | **15/15 (100%)** |
