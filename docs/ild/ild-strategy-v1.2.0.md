...
| **기반 문서** | LLD STRATEGY v1.2.0, HLD v1.2.0 (4.3, 5.3.2, 6.3, 7.5), SRS v1.2.0 (FR-003, FR-004, FR-005, FR-006, FR-010, FR-016, FR-017, FR-018, CON-006, CON-007, CON-008, NFR-008) |
| **관련 티켓** | TICKET-077-ILD-STRATEGY |

---

## v1.2.0 변경 로그 (요약)

- 전략 C(`ThreeMinuteBuyTrailingThenSellTrailingStrategy`) 구현 절차 추가
- 전략 C 핵심 규칙 반영: **3분봉 / 09:03 시가 기준가 / 1.0% 하락 / 0.2% 반등 / 해당 3분봉 종가 전액 매수**
- 신규 전략 공통 제약 적용 범위를 A/B/C로 통합: 당일 손절 미적용, 매수/매도 교대, 연속 매수 금지
- `StrategyRegistry` 및 WebAPI/Frontend 전략 ID 계약을 6개 전략 기준으로 정합화
- LLD→ILD 추적성 매트릭스를 FR-018, CON-008 포함 기준으로 갱신
...
본 문서는 `lld-strategy-v1.2.0.md`를 실제 코드로 구현하기 위한 **구현 수준(ILD)** 명세를 제공한다. 특히 전략 C를 포함한 전략 1/2/3/A/B/C, `BaseStrategy`, `StrategyRegistry`, 전략 ID 계약을 함수/시그니처/반환/예외/상태 전이 단위로 구체화하여 초급 개발자가 외부 문서 없이 구현 가능하도록 한다.

### 1.2 범위 (In-Scope)

- 전략 공통 템플릿 메서드(`evaluate`)와 상태 객체(`TradeContext`) 구현
- 전략 1/2/3/A/B/C의 매수·매도·손절 규칙 구현
- 전략 C 구현: 3분봉 규칙, 09:03 기준가, 하락·반등 진입, 종가 체결
- 신규 전략 공통 제약(A/B/C) 구현: **당일 손절 미적용(CON-006), 매수/매도 교대(CON-007), 연속 매수 금지(CON-007)**
- `StrategyRegistry` 등록/조회/기본 등록 6전략 계약
- WebAPI/Frontend 전략 ID와 StrategyRegistry 이름의 6전략 통합 계약
- `TradeSignal` 생성 및 `SimulationEngine`/`TradeExecutor` 핸드오프 계약
- LLD v1.2.0 항목에 대한 구현 추적성 확보

### 1.3 비범위 (Out-of-Scope)

- Yahoo Finance 조회 및 분봉 집계 알고리즘 자체 구현
- RSI 계산 알고리즘 자체 구현
...
src/
  strategy/
    ...
    three_minute_buy_trailing_then_sell_trailing_strategy.py
...
| `strategy_input_validator.py` | 캔들/RSI 정합성, interval, 기준 시각 검증 |
...
| `three_minute_buy_trailing_then_sell_trailing_strategy.py` | 전략C(3분봉) 로직 구현 |
...
> 구현 계약: 전략 1/2/3은 최대 1건 신호를 반환하고, 전략 A/B/C는 당일 복수 사이클에 따라 0..N건을 반환할 수 있다.
...
| `meta` | dict[str, Any] | Y | 추적 정보 (`highest_profit_rate`, `preserve_ratio`, `trade_cycles`, `alternation_blocked`, `entry_interval`, `reference_time`) |
...
| 필드 | 타입 | 제약 |
|------|------|------|
| `timestamp` | datetime(tz-aware) | KST, 오름차순, 중복 없음 |
| `open/high/low/close` | Decimal/float | `high >= low`, `open/close ∈ [low, high]` |
| `volume` | int | `>= 0` |
| `interval` | string(논리 계약) | 전략1/2/3/A/B=`5m`, 전략C=`3m` |
...
| 전략 | 필수 캔들 | 이유 |
|------|-----------|------|
| 전략1 | 09:05, 15:05 | 09:05 매수, 15:05 손절 체결가 산출 |
| 전략2 | 09:05, 15:00 | 기준가 및 손절 컷오프 |
| 전략3 | 09:05, 15:00 | 기준가 및 손절 컷오프 |
| 전략A | 09:05 | RSI 기반 일중 순회 정합성 |
| 전략B | 09:05 | 기준가 설정 |
...
| `register_defaults()` | 없음 | `None` | 기본 6전략 모두 등록 |
...
6. `three_minute_buy_trailing_then_sell_trailing`

### 3.6 WebAPI/Frontend 전략 ID 통합 계약

| 계층 | 필드/타입 | 허용 ID |
|------|-----------|---------|
| WebAPI 요청 바디 | `strategy: string` | 6전략 ID 전체 허용 |
| Frontend 도메인 타입 | `StrategyId` | 6전략 ID 전체 허용 |
| Frontend validator | `ALLOWED_STRATEGIES` | 6전략 ID와 일치 |
...
| `initialize_day` | `(reference_candle: Series, seed_money: Decimal) -> TradeContext` | 전략별 초기 상태 구성 | `StrategyInputError` |
...
| `can_open_new_position` | `(context: TradeContext) -> bool` | 교대 제약 평가 | `AlternationRuleViolationError` |

#### 4.1.2 `evaluate` 내부 단계 (반드시 준수)

1. `validate_inputs` 수행 (`required_interval`, `required_reference_time` 전달).
2. 기준 시각 캔들 조회 후 `initialize_day` 호출.
3. 캔들 시간순 순회.
4. 미보유 상태에서 `should_buy`와 `can_open_new_position` 평가.
5. 매수 수량은 `floor(seed_money / buy_price)`로 계산.
6. `buy_quantity == 0`이면 매수 스킵(`meta.insufficient_seed=true`).
7. 보유 상태에서 수익률/최고수익률/보전율 갱신.
8. `should_sell` True면 `PROFIT_PRESERVE` 신호 append.
9. `allow_same_day_stop_loss=True`인 경우에만 `should_stop_loss` 분기 허용.
10. `allow_multiple_round_trips=False`면 첫 매도 후 루프 종료.
...
| `required_interval() -> str` | `5m` |
| `required_reference_time() -> str` | `09:05` |
...
| `required_interval` | `() -> str` | `5m` |
| `required_reference_time` | `() -> str` | `09:05` |
| `initialize_day` | `(reference_candle: Series, seed_money: Decimal) -> TradeContext` | `allow_same_day_stop_loss=False`, `allow_multiple_round_trips=True`, `last_action=NONE` |
...
| `should_stop_loss` | `(candle: Series, context: TradeContext) -> bool` | **항상 False** |
...
| `required_interval` | `() -> str` | `5m` |
| `required_reference_time` | `() -> str` | `09:05` |
| `initialize_day` | `(reference_candle: Series, seed_money: Decimal) -> TradeContext` | `reference_price=close@09:05`, `allow_same_day_stop_loss=False`, `allow_multiple_round_trips=True` |
| `should_buy` | `(candle: Series, context: TradeContext, rsi_value: float|None=None) -> bool` | 전략2 하락/반등 충족 + 교대 규칙 통과 시 True |
| `should_sell` | `(candle: Series, context: TradeContext) -> bool` | trailing 시작 후 `preserve_ratio <= 80`이면 True |
| `should_stop_loss` | `(candle: Series, context: TradeContext) -> bool` | **항상 False** |
| `can_open_new_position` | `(context: TradeContext) -> bool` | `is_bought=False` AND `last_action != BUY`일 때 True |

### 4.7 `ThreeMinuteBuyTrailingThenSellTrailingStrategy` (전략C)

#### 4.7.1 클래스/메서드 시그니처 및 반환 계약

| 메서드 | 시그니처 | 반환 계약 |
|--------|----------|-----------|
| `name` | `() -> str` | 항상 `three_minute_buy_trailing_then_sell_trailing` |
| `required_interval` | `() -> str` | `3m` |
| `required_reference_time` | `() -> str` | `09:03` |
| `initialize_day` | `(reference_candle: Series, seed_money: Decimal) -> TradeContext` | `reference_price=open@09:03`, `low_point=None`, `allow_same_day_stop_loss=False`, `allow_multiple_round_trips=True`, `last_action=NONE` |
| `should_buy` | `(candle: Series, context: TradeContext, rsi_value: float|None=None) -> bool` | 09:03 이후 `1.0% 하락 시작` + `전저점 대비 0.2% 반등` + `can_open_new_position=True`면 True |
| `should_sell` | `(candle: Series, context: TradeContext) -> bool` | trailing 시작 후 `preserve_ratio <= 80`이면 True |
...
1. `initialize_day`에서 기준가(`reference_price`)를 **09:03 3분봉 시가(open)** 로 설정한다.
2. `should_buy`는 `time > 09:03` 구간에서 하락률을 계산한다.
3. 기준가 대비 하락률 `>= 1.0`이 최초 성립하면 전저점 추적을 시작한다.
4. 추적 구간에서 더 낮은 종가가 나오면 `low_point`를 갱신한다.
5. 반등률 `((close-low_point)/low_point)*100 >= 0.2`이면 매수 후보가 된다.
6. 매수 체결 가격은 **해당 3분봉 종가(close)** 이다.
7. 매수 수량은 `floor(seed_money / buy_price)`이고 0이면 매수 스킵한다.
8. 매수 후 매도는 전략1의 trailing 규칙과 동일하게 처리한다.
9. `should_stop_loss`는 모든 캔들에서 False를 반환한다.
10. 체결 시 `BUY -> SELL -> BUY -> SELL` 교대만 허용한다.

#### 4.7.3 예외/스킵 정책

| 상황 | 처리 |
|------|------|
| 09:03 캔들 누락 | `StrategyReferenceCandleMissingError(E-ST-011)` |
| 3분봉 interval 불일치 | `StrategyIntervalMismatchError(E-ST-010)` |
| 교대 위반(`last_action==BUY`에서 BUY 시도) | `AlternationRuleViolationError(E-ST-009)` 또는 매수 스킵 |
| 시드머니 부족(`buy_quantity==0`) | `InsufficientSeedMoneyError(E-ST-007)` 또는 매수 스킵 |

### 4.8 `StrategyRegistry`
...
sequenceDiagram
    ...
    REG-->>SIM: strategy instance (1/2/3/A/B/C)
    SIM->>STR: evaluate(daily_candles, rsi_data, seed_money)
    STR->>VAL: validate_inputs(daily_candles, required_interval, required_reference_time)
    VAL-->>STR: OK
...
4. 전략 A/B/C는 `allow_same_day_stop_loss=False`이므로 손절 분기 호출 금지.
5. 전략 A/B/C는 `BUY->SELL->BUY->SELL` 교대 외 거래 순서 금지.
6. 전략 C의 기준 시각은 09:03이며 3분봉으로만 실행한다.
...
| CON-007 교대 강제 | `last_action==BUY` 상태에서 BUY 금지 | 매수 스킵 또는 `AlternationRuleViolationError` |
| CON-007 연속 매수 금지 | `last_action != BUY`일 때만 신규 BUY 가능 | 매수 거절 |
| CON-007 보유 중 재매수 금지 | `is_bought=True`면 `should_buy=False` | 매수 스킵 |
| CON-008 전략C 3분봉 규칙 | `required_interval=3m`, `required_reference_time=09:03` | 검증 실패 시 즉시 오류 |
...
| `E-ST-009` | `AlternationRuleViolationError` | A/B/C에서 `BUY->BUY` 시도 | `strategy_alternation_violation` | 시도 거절 후 계속 진행 |
| `E-ST-010` | `StrategyIntervalMismatchError` | 전략 요구 interval 불일치 | `strategy_interval_mismatch` | 즉시 실패 |
| `E-ST-011` | `StrategyReferenceCandleMissingError` | 09:05/09:03 기준 캔들 누락 | `strategy_reference_candle_missing` | 즉시 실패 |
...
| 시드머니 부족 | `floor(seed_money/buy_price)==0` | 해당 시점 BUY 포기, 다음 캔들 진행 | 동일 거래일 평가 지속 |
| 교대 규칙 위반 (A/B/C) | `last_action==BUY` 상태 BUY 시도 | `meta.alternation_blocked=true` 기록 | 동일 거래일 평가 지속 |
| 전략C 기준 시각 누락 | 09:03 캔들 조회 실패 | `E-ST-011` 발생 | 해당 거래일 실패 |
| 전략C interval 혼입 | 3분봉 외 데이터 유입 | `E-ST-010` 발생 | 해당 거래일 실패 |
...
2. `last_action == BUY` 상태에서는 매수 시도를 차단한다.
...
### 8.7 전략C 구현 기준 (FR-018, CON-006, CON-007, CON-008)

1. 기준가 = **09:03 3분봉 시가(open)**.
2. 09:03 이후 3분봉 종가 기준으로 하락률을 계산한다.
3. 하락률 `>= 1.0` 최초 성립 시 전저점 추적을 시작한다.
4. 추적 구간에서 더 낮은 종가가 나오면 `low_point`를 갱신한다.
5. 반등률 `>= 0.2`이면 해당 **3분봉 종가(close)** 로 전액 매수한다.
6. 매수 수량은 `floor(seed_money / buy_price)`이며 0이면 매수 스킵한다.
7. 매수 후 trailing 매도 규칙(1.0% 시작, 보전율 80% 이하 매도)을 적용한다.
8. `should_stop_loss()`는 항상 False를 반환한다.
9. 체결 순서는 `BUY->SELL->BUY->SELL`만 허용한다.

### 8.8 확장성 구현 기준 (FR-006, NFR-008)
...
4. 확장 시 기존 전략 1/2/3/A/B 결과 불변 회귀를 포함한다.
...
function evaluate(daily_candles, rsi_data, seed_money):
    ...
    ref = must_find_candle(daily_candles, required_reference_time())
    context = initialize_day(ref, seed_money)
    signals = []
...
### 9.2 전략C (`ThreeMinuteBuyTrailingThenSellTrailingStrategy`) 구현 수도코드

```text
class ThreeMinuteBuyTrailingThenSellTrailingStrategy(BaseStrategy):

    function name() -> str:
        return "three_minute_buy_trailing_then_sell_trailing"

    function required_interval() -> str:
        return "3m"

    function required_reference_time() -> str:
        return "09:03"

    function initialize_day(candle_0903, seed_money) -> TradeContext:
        context = build_default_context(candle_0903.date, seed_money)
...
function register_defaults():
    ...
    register(ThreeMinuteBuyTrailingThenSellTrailingStrategy())
```
...
| 2.6 | 전략 A 인터페이스 | 4.5, 8.5 | RSI<=30 진입, 무손절, 교대 제약 |
| 2.7 | 전략 B 인터페이스 | 4.6, 8.6 | 하락/반등 진입, 무손절, 교대 제약 |
| 2.8 | 전략 C 인터페이스(3분봉/09:03/open 기준) | 4.7, 8.7, 9.2 | 하락/반등 진입, 종가 체결, 무손절, 교대 |
| 2.9 | `StrategyRegistry` 6전략 기본 등록 | 3.5, 4.8, 9.3 | 6전략 ID 등록/조회/중복 방지 |
| 2.10 | StrategyRegistry/WebAPI/Frontend 전략 ID 계약 | 3.6, 5.1 | 6전략 ID 통합 계약 |
| 4.3 | A/B/C 공통 제약(CON-006/007) | 6.3, 7.1, 8.5~8.7 | 손절 비활성, 교대 강제, 연속 매수 금지 |
| 4.4 | 전략 C 상세 규칙(FR-018, CON-008) | 4.7, 8.7, 9.2 | 09:03 기준가, 1.0% 하락, 0.2% 반등, 3분봉 종가 매수 |
| 5.1 | 예외 분류 및 처리 | 7.1~7.3 | 코드별 예외/복구/전파 규칙 |
| 6.1/6.2 | interval/기준 시각 검증 | 3.3, 3.4, 7.1 | `E-ST-010`, `E-ST-011` 검증 계약 |
| 9 | 요구사항 추적(FR-018, CON-008 포함) | 8.7, 10 | 전략 C 구현 기준 연결 |
...
1. `errors.py`, `constants.py`, `models.py`에 전략 C 및 interval/reference 예외 코드를 추가한다.
2. `BaseStrategy.evaluate`를 `required_interval`, `required_reference_time` 기반으로 정리한다.
3. 전략 C 파일을 생성하고 `name`, `required_interval`, `required_reference_time`, `initialize_day`, `should_buy`, `should_sell`, `should_stop_loss`, `can_open_new_position`를 구현한다.
4. `StrategyRegistry.register_defaults`를 6전략 기준으로 갱신한다.
5. WebAPI `strategy` 허용값과 Frontend `StrategyId`/validator/options를 6전략으로 확장한다.
6. 기존 전략 1/2/3/A/B 회귀 확인(동일 입력/동일 출력)을 수행한다.
7. 전략 C 경계값 시나리오(1.0%, 0.2%, 09:03, 3분봉 interval, 무손절, 교대)를 단위 테스트로 검증한다.

### 11.2 구현 완료 기준 (Definition of Done)

- 전략 C 클래스가 문서 시그니처와 동일한 반환 계약을 충족한다.
- 전략 C에서 `should_stop_loss()`는 항상 False를 반환한다.
- 전략 A/B/C에서 연속 BUY가 발생하지 않고 `BUY->SELL` 교대가 보장된다.
- `StrategyRegistry.list_all()`과 WebAPI/Frontend 허용 전략 ID 집합이 100% 일치한다.
- LLD→ILD 추적성 항목(FR-018, CON-008 포함)이 누락 없이 연결되어 검토 PASS를 획득한다.
...
| 1.1.0 | 2026-02-16 | 전략 A/B 구현 절차, 무손절/교대 제약, 5전략 ID 통합 계약, LLD→ILD 추적성 추가 | ILD 담당 에이전트 |
| 1.2.0 | 2026-02-16 | 전략 C 구현 절차 추가(3분봉/09:03/open 기준/하락·반등/종가 체결), A/B/C 제약 통합, 6전략 ID 계약 정합화, FR-018/CON-008 추적성 반영 | ILD 담당 에이전트 |
