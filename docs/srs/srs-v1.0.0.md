...
| **문서 버전** | 1.0.0 |
| **작성일** | 2026-02-15 |
| **기반 문서** | 유저스토리 v1.0.0, 사용자 인터뷰 (2026-02-15) |
| **형식** | IEEE 830 기반 |
| **티켓** | TICKET-002 |
...
- 3가지 투자 전략(매도 Trailing Stop, 매수+매도 Trailing Stop, RSI 매수 Trailing Stop)에 대한 시뮬레이션 실행
...
| 투자 전략 | 매도 Trailing Stop, 매수+매도 Trailing Stop, RSI 매수 Trailing Stop, 전략 확장 |
...
#### FR-003: 매도 Trailing Stop 전략 (전략 1)
...
**설명**: 시스템은 매도 Trailing Stop 전략으로 시뮬레이션을 실행할 수 있어야 한다. 매일 오전 9시 5분에 매수하고, 일정 수익 확보 후 이익보전 추적 매도를 수행한다.
...
#### FR-004: 매수+매도 Trailing Stop 전략 (전략 2)
...
**설명**: 시스템은 매수 Trailing Stop과 매도 Trailing Stop을 결합한 전략으로 시뮬레이션을 실행할 수 있어야 한다. 하락 후 반등 시점에 매수하고, 전략 1과 동일한 이익보전 매도를 수행한다.
...
#### FR-005: RSI 매수 Trailing Stop 전략 (전략 3)
...
**설명**: 시스템은 전략 2(매수+매도 Trailing Stop)에 RSI 30 이하 조건을 추가한 전략으로 시뮬레이션을 실행할 수 있어야 한다.
...
2. 신규 전략 추가 시 기존 전략(전략 1, 2, 3)의 동작에 영향을 주지 않는다.
...
3. 각 거래일에 선택된 전략(전략 1, 2, 3 중 하나)의 매수/매도 조건에 따라 거래를 수행한다.
...
### BR-005: 전략 1 — 매도 Trailing Stop 매수/매도 조건
...
### BR-006: 전략 2 — 매수+매도 Trailing Stop 매수/매도 조건
...
### BR-007: 전략 3 — RSI 매수 Trailing Stop 매수/매도 조건
...
| 항목 | 수량 |
|------|------|
| 총 유저스토리 수 | 17 (US-001 ~ US-017) |
| 기능 요구사항 수 | 15 (FR-001 ~ FR-015) |
| 비기능 요구사항 수 | 8 (NFR-001 ~ NFR-008) |
| 총 요구사항 수 | 23 |
| 유저스토리 커버리지 | 17/17 (100%) |
