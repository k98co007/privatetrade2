...
| **문서 버전** | 1.2.0 |
| **작성일** | 2026-02-16 |
| **기반 문서** | 유저스토리 v1.2.0, 사용자 인터뷰 (2026-02-15, 2026-02-16) |
| **형식** | IEEE 830 기반 |
| **티켓** | TICKET-074 |
...
- 2026-02-16 3차 인터뷰 기반 신규 전략C(US-020) 요구사항 추가(FR-018)
- 전략C 핵심 규칙 반영: 3분봉, 09:03 기준가, 1% 하락 후 전저점 추적, 전저점 대비 0.2% 상승 시 3분봉 종가 전액 매수
- 신규 전략 공통 제약 반영 확대: 당일 손절 미적용, 연속 매수 금지(매수/매도 교대) 적용 대상에 전략C 포함
- 기존 전략(FR-003/FR-004/FR-005)과 신규 전략(FR-016/FR-017/FR-018)의 적용 범위 분리
- 추적 매트릭스에 US-020 ↔ FR-018 매핑 및 커버리지 수치 갱신
...
시스템이 제공하는 주요 기능:
- 야후 파이낸스를 통한 코스피 종목 5분/3분 분봉 데이터 및 RSI 지표 조회
- 6가지 투자 전략(기존 전략 3종 + 신규 전략 3종)에 대한 시뮬레이션 실행
...
| **3분 분봉** | 3분 간격으로 기록된 주식의 시가(Open), 고가(High), 저가(Low), 종가(Close), 거래량(Volume) 데이터 |
...
| **전저점** | 전략이 사용하는 분봉(5분 또는 3분) 기준으로 추적하는 최저 가격 지점 |
| **시드머니** | 시뮬레이션에 사용하는 초기 투자 원금. 거래 결과에 따라 누적 증감 |
| **손절매** | 일정 시간 또는 조건에 도달했을 때 추가 손실 방지를 위해 현재가에 매도하는 행위 |
| **기준가** | 각 전략에서 매수/매도 판단의 기준이 되는 가격. 전략 2/3/B는 오전 9시 5분, 전략 C는 오전 9시 3분 가격 |
...
| 기능 영역 | 주요 기능 |
|-----------|----------|
| 시세 데이터 수집 | 코스피 종목 5분/3분 분봉 데이터 조회, RSI 지표 조회 |
| 투자 전략 | 기존 전략(전략1/전략2/전략3) + 신규 전략(전략A/전략B/전략C), 전략 확장 |
...
| **시세 데이터** | Yahoo Finance 5분/3분 분봉 데이터, 최근 60일 |
...
| CON-002 | Yahoo Finance의 5분/3분 분봉 데이터 조회 가능 기간은 최근 60일로 제한된다 |
...
| CON-006 | 신규 전략(전략A, 전략B, 전략C)은 당일 손절매(15:00 강제 손절)를 적용하지 않는다 |
| CON-007 | 신규 전략(전략A, 전략B, 전략C)은 미청산 포지션 보유 중 추가 매수를 금지하며, 거래 순서는 매수→매도 교대만 허용한다 |
| CON-008 | 전략C는 3분 분봉 기준으로만 동작하며, 기준가는 오전 9시 3분(09:03) 가격으로 고정한다 |

### 2.6 가정

| ID | 가정 |
|----|------|
| ASM-001 | Yahoo Finance API가 한국 코스피 종목의 5분/3분 분봉 데이터를 안정적으로 제공한다 |
...
**설명**: 시스템은 2026-02-15 정의의 기존 매도 Trailing Stop 전략(전략 1)으로 시뮬레이션을 실행할 수 있어야 한다. 본 요구사항의 손절매 규칙은 전략 1에만 적용되며, 신규 전략A/B/C에는 적용되지 않는다.
...
**설명**: 시스템은 2026-02-15 정의의 기존 매수+매도 Trailing Stop 전략(전략 2)으로 시뮬레이션을 실행할 수 있어야 한다. 본 요구사항의 손절매 규칙은 전략 2에만 적용되며, 신규 전략B(FR-017) 및 전략C(FR-018)에는 적용되지 않는다.
...
**설명**: 시스템은 2026-02-15 정의의 기존 RSI 매수 Trailing Stop 전략(전략 3)으로 시뮬레이션을 실행할 수 있어야 한다. 본 요구사항의 손절매 규칙은 전략 3에만 적용되며, 신규 전략A(FR-016), 전략B(FR-017), 전략C(FR-018)에는 적용되지 않는다.
...
#### FR-018: 3분봉 매수 Trailing Buy + 매도 Trailing Stop 조합 전략 (전략 C, 신규 전략)
...
**설명**: 시스템은 2026-02-16 3차 인터뷰에서 정의된 신규 전략C를 지원해야 한다. 3분봉 기준 하락 후 반등 진입을 수행하고, 매수 후에는 매도 Trailing Stop으로 청산한다.

**상세 요구사항**:

*기준가 설정 및 매수 조건:*
1. 시스템은 각 거래일의 오전 9시 3분(09:03) 3분 분봉 시가를 기준가로 설정한다.
2. 오전 9시 3분 이후 3분 분봉 종가가 기준가 대비 1.0% 이상 하락하면 전저점 추적을 시작한다.
3. 전저점 추적 시작 후, 3분 분봉 단위로 최저가를 갱신한다.
4. 전저점 대비 현재가(3분 분봉 종가)가 0.2% 이상 상승하면 해당 3분 분봉 종가로 전액 매수한다.
5. 매수 수량은 현재 시드머니 전액을 매수 가격으로 나눈 최대 주식 수(정수)이다.
...
2. 신규 전략 추가 시 기존 전략(전략 1, 2, 3)과 신규 전략(전략 A, B, C)의 동작에 영향을 주지 않는다.
...
3. 각 거래일에 선택된 전략(전략 1, 2, 3, A, B, C 중 하나)의 매수/매도 조건에 따라 거래를 수행한다.
4. 매 거래일 시뮬레이션은 오전 9시(09:00) ~ 오후 3시 30분(15:30) 범위의 분봉 데이터를 사용한다.
   - 전략 1/2/3/A/B: 5분 분봉
   - 전략 C: 3분 분봉
...
2. 시뮬레이션 시작 시 다음 항목을 입력/선택할 수 있다:
   ...
   - 투자 전략 선택 (전략 1, 전략 2, 전략 3, 전략 A, 전략 B, 전략 C 중 택 1)
...
4. 전략별 매수/매도 조건의 퍼센트 기준(1.0%, 80%, 0.2%, RSI 30)과 시간 기준(09:05, 09:03), 분봉 기준(5분, 3분)이 정확히 적용되어야 한다.
...
| 3분 분봉 (Intraday 3min) | 3분 간격 OHLCV 데이터 | 전략C 매매 판단 기초 데이터 |
| RSI 지표 | 상대강도지수 | 전략 3 및 전략A 매수 조건 판단 |
...
### BR-010: 전략 C(신규 전략) — 3분봉 매수 Trailing Buy + 매도 Trailing Stop 조합 조건

| 항목 | 내용 |
|------|------|
| **규칙 ID** | BR-010 |
| **기준가** | 오전 9시 3분(09:03) 3분 분봉 시가 |
| **하락 추적 시작** | 기준가 대비 현재가 하락률 ≥ 1.0% |
| **전저점 추적** | 하락 추적 시작 후 3분 분봉 최저가 갱신 |
| **매수 조건** | 전저점 대비 현재가 상승률 ≥ 0.2%일 때 해당 3분 분봉 종가로 전액 매수 |
| **매도 조건** | 전략 1(BR-005)의 이익보전 매도 조건과 동일 |
| **당일 손절매** | 적용하지 않음 |
| **추가 매수 제한** | 미청산 포지션 보유 시 추가 매수 금지 |
| **거래 순서 제약** | 매수→매도 교대만 허용, 연속 매수 금지 |
...
| US-020 | 매수 Trailing Buy + 매도 Trailing Stop 조합 전략 (전략C) | FR-018 | 3분봉 매수 Trailing Buy + 매도 Trailing Stop 조합 전략 |
...
| 항목 | 수량 |
|------|------|
| 총 유저스토리 수 | 20 (US-001 ~ US-020) |
| 기능 요구사항 수 | 18 (FR-001 ~ FR-018) |
| 비기능 요구사항 수 | 8 (NFR-001 ~ NFR-008) |
| 총 요구사항 수 | 26 |
| 유저스토리 커버리지 | 20/20 (100%) |
