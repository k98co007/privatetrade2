...
| **기반 문서** | HLD v1.1.0 (4.3, 5.x, 6.x, 7.x), SRS v1.1.0 (FR-003, FR-004, FR-005, FR-006, FR-016, FR-017, CON-006, CON-007, NFR-008) |
| **관련 티켓** | TICKET-069-LLD-STRATEGY |

---

## v1.1.0 변경 로그 (요약)

- 2026-02-16 인터뷰 기반 신규 전략 2건(전략 A, 전략 B) LLD 상세화
- 전략 A/B 공통 제약 반영: 당일 손절 미적용, 매수/매도 교대(연속 매수 금지)
- `BaseStrategy`/`StrategyRegistry` 인터페이스를 5전략 기준으로 확장 정의
- 기존 전략 1/2/3의 동작 불변(손절 규칙, 1일 1매수/1매도) 하위 호환성 명시
...
본 문서는 HLD 4.3(전략 엔진), 5.x(모듈 간 인터페이스), 6.x(거래일 단위 데이터 흐름), 7.x(전략 확장 구조)를 구현 가능한 수준으로 상세화한다. 대상은 `BaseStrategy`, 전략 5종 구현체, `StrategyRegistry`이며 FR-003~FR-006, FR-016~FR-017 및 NFR-008을 직접 충족한다.

### 1.2 범위 (In-Scope)

- `BaseStrategy` 추상 인터페이스 및 템플릿 실행 흐름 정의
- 기존 전략 3종과 신규 전략 A/B의 매수/매도/손절 규칙 상세화
- 전략 선택/실행 및 거래 시그널 생성 시퀀스 정의
- 전략 A/B 공통 제약(당일 손절 미적용, 매수/매도 교대) 상태 전이 규칙 정의
- 전략 입력 데이터(캔들/RSI/시간대) 검증 규칙 정의
- 전략 계층 예외 모델 및 오류 전파 규칙 정의
- 신규 전략 추가 절차와 기존 전략 호환성 보호 규칙 정의
...
|------|------|------|
| `trade_date` | date | 거래일 |
| `reference_price` | Decimal? | 기준가(09:05 종가, 전략2/3/B 사용) |
| `low_point` | Decimal? | 하락 추적 중 전저점 |
| `buy_price` | Decimal? | 현재 보유 포지션 매수 단가 |
| `buy_quantity` | int | 현재 보유 수량 |
| `buy_datetime` | datetime? | 현재 포지션 매수 시각 |
| `is_bought` | bool | 현재 포지션 보유 여부 |
| `is_trailing_started` | bool | 현재 포지션 이익보전 추적 시작 여부 |
| `highest_profit_rate` | Decimal | 현재 포지션 최고 수익률(%) |
| `latest_profit_rate` | Decimal | 최신 수익률(%) |
| `sell_reason` | SellReason? | 최근 매도 사유 |
| `last_action` | ActionType | 최근 실행 액션 (`NONE`,`BUY`,`SELL`) |
| `trade_cycles` | int | 당일 완료된 매수→매도 사이클 수 |
| `allow_same_day_stop_loss` | bool | 당일 손절 적용 여부 (전략1/2/3=True, A/B=False) |
| `allow_multiple_round_trips` | bool | 당일 복수 사이클 허용 여부 (전략1/2/3=False, A/B=True) |
...
| `meta` | map<string, any> | 추적 지표(최고수익률, 이익보전율, RSI, 사이클 수, 제약 위반 플래그) |

#### 2.1.3 `SellReason` 열거형

- `PROFIT_PRESERVE`: 이익보전율 80% 이하 하락 매도
- `STOP_LOSS`: 15:00 조건 기반 손절매(기존 전략 전용)
...
- 전략별 제약(손절 적용 여부, 매수/매도 교대) 불변식 검증

#### 2.2.2 인터페이스

| 메서드 시그니처 | 파라미터 | 반환값 | 예외 |
|----------------|----------|--------|------|
| `evaluate(daily_candles, rsi_data, seed_money) -> list[TradeSignal]` | 거래일 5분봉, RSI, 시드머니 | 당일 거래 시그널 목록(기존 전략은 최대 1건) | `StrategyInputError`, `StrategyExecutionError` |
...
2. 모든 전략은 포지션 보유 중 추가 매수를 금지한다.
3. 전략 1/2/3은 당일 최대 1회 매수·1회 매도만 허용한다.
4. 전략 A/B는 `매수→매도→매수→매도` 교대만 허용하며 `매수→매수`는 금지한다.
5. 매수 수량은 `floor(seed_money / buy_price)`로 계산한다.
6. `buy_quantity == 0`이면 해당 매수 시도는 `NO_TRADE`로 기록하고 다음 캔들로 진행한다.
7. `allow_same_day_stop_loss=False`인 전략(A/B)은 `should_stop_loss()`가 항상 False여야 한다.

### 2.3 `SellTrailingStopStrategy` (전략 1, 기존)
...
| `should_stop_loss()` | 15:00 컷오프까지 `is_trailing_started == False`이면 True |

### 2.4 `BuySellTrailingStopStrategy` (전략 2, 기존)
...
| `should_buy()` | 하락 조건 성립 후 `rebound_rate >= 0.2`이면 True |
| `should_sell()` | 전략1과 동일 |
| `should_stop_loss()` | 전략1과 동일 |

### 2.5 `RSIBuySellTrailingStopStrategy` (전략 3, 기존)
...
### 2.6 `RSIOnlyTrailingStopStrategy` (전략 A, 신규)

#### 2.6.1 책임

- RSI 값이 30 이하인 시점에서만 매수
- 매수 후 매도는 이익보전 Trailing Stop 규칙 적용
- 당일 손절 미적용, 매수/매도 교대 규칙 준수
...
| `should_sell()` | 전략1과 동일(이익보전율 80% 이하 매도) |
| `should_stop_loss()` | 항상 False 반환 |
| `can_open_new_position()` | `is_bought=False` AND `last_action != BUY`이면 True |

### 2.7 `BuyTrailingThenSellTrailingStrategy` (전략 B, 신규)

#### 2.7.1 책임

- 전략2의 하락(1.0%)/반등(0.2%) 매수 진입 규칙 사용
- 매수 후 매도는 이익보전 Trailing Stop 규칙 적용
- 당일 손절 미적용, 매수/매도 교대 규칙 준수
...
| `should_buy()` | 전략2의 하락/반등 조건 AND `can_open_new_position(context)` |
| `should_sell()` | 전략1과 동일 |
| `should_stop_loss()` | 항상 False 반환 |
| `can_open_new_position()` | `is_bought=False` AND `last_action != BUY`이면 True |

### 2.8 `StrategyRegistry`
...
- `rsi_only_trailing_stop` → `RSIOnlyTrailingStopStrategy` (전략 A)
- `buy_trailing_then_sell_trailing` → `BuyTrailingThenSellTrailingStrategy` (전략 B)

---

## 3. 전략 선택/실행 시퀀스

### 3.1 전략 선택 및 실행 시퀀스 (5전략)
...
### 3.2 전략 A 실행 시퀀스 (RSI 기반 + 교대 + 무손절)

```mermaid
sequenceDiagram
    participant SIM as SimulationEngine
    participant SA as RSIOnlyTrailingStopStrategy

    SIM->>SA: evaluate(candles, rsi, seed)
    loop 각 5분봉
        SA->>SA: is_bought ?
        alt 미보유
            SA->>SA: RSI<=30 && can_open_new_position?
            alt True
                SA->>SA: BUY 체결, last_action=BUY
            else False
                SA->>SA: 대기
            end
        else 보유
            SA->>SA: 수익률/이익보전율 갱신
            SA->>SA: preserve_ratio<=80 ?
            alt True
                SA->>SA: SELL(PROFIT_PRESERVE), last_action=SELL, trade_cycles+1
            else False
                SA->>SA: 보유 유지
            end
        end
        Note over SA: should_stop_loss()는 항상 False
    end
    SA-->>SIM: list[TradeSignal]
```

### 3.3 전략 B 실행 시퀀스 (하락/반등 + 교대 + 무손절)

```mermaid
sequenceDiagram
    participant SIM as SimulationEngine
    participant SB as BuyTrailingThenSellTrailingStrategy

    SIM->>SB: evaluate(candles, rsi, seed)
    SB->>SB: initialize_day(09:05 기준가)
    loop 각 5분봉
        alt 미보유
            SB->>SB: 기준가대비 1.0% 하락 여부
            SB->>SB: 전저점 갱신
            SB->>SB: 전저점대비 0.2% 반등 && can_open_new_position?
            alt True
                SB->>SB: BUY 체결, last_action=BUY
            else False
                SB->>SB: 대기
            end
        else 보유
            ...
            alt preserve_ratio<=80
                SB->>SB: SELL(PROFIT_PRESERVE), last_action=SELL, trade_cycles+1
            else 미충족
                SB->>SB: 보유 유지
            end
        end
        Note over SB: should_stop_loss()는 항상 False
    end
    SB-->>SIM: list[TradeSignal]
```
...
### 4.2 전략 1: 매도 Trailing Stop (FR-003, 기존)
...
### 4.3 전략 2: 매수+매도 Trailing Stop (FR-004, 기존)
...
### 4.4 전략 3: RSI 매수 Trailing Stop (FR-005, 기존)
...
### 4.5 전략 A: RSI<=30 매수 + 매도 Trailing Stop (FR-016, 신규)

#### 매수 규칙

1. 5분봉 단위로 RSI 값을 지속 확인한다.
2. RSI 값이 `<= 30`이고 포지션이 없을 때만 매수한다.
3. 직전 액션이 `BUY`이면 신규 매수는 차단한다(교대 규칙).
4. 수량은 `floor(seed_money / buy_price)`.

#### 매도 규칙

1. 매수 후 수익률이 최초 1.0% 이상 도달하면 trailing 시작.
2. trailing 시작 후 이익보전율 `<= 80`이면 즉시 매도한다.
3. 매도 체결 시 `last_action=SELL`, `trade_cycles += 1`로 상태 전이한다.

#### 손절 규칙

1. 당일 손절(15:00 강제 손절)을 적용하지 않는다.
2. `should_stop_loss()`는 모든 캔들에서 False를 반환해야 한다.

### 4.6 전략 B: 매수 Trailing + 매도 Trailing Stop (FR-017, 신규)

#### 매수 규칙

1. 09:05 종가를 기준가로 설정한다.
2. 기준가 대비 1.0% 이상 하락 시 전저점 추적을 시작한다.
3. 전저점 대비 0.2% 이상 반등 시 포지션이 없고 직전 액션이 `BUY`가 아닐 때 매수한다.
4. 수량은 `floor(seed_money / buy_price)`.

#### 매도 규칙

1. 매수 후 수익률 1.0% 도달 시 trailing 시작.
2. 이익보전율 `<= 80`이면 즉시 매도한다.
3. 매도 체결 시 `last_action=SELL`, `trade_cycles += 1`로 상태 전이한다.
...
1. **무손절 제약(CON-006)**: 전략 A/B는 15:00 손절 분기를 절대 수행하지 않는다.
2. **교대 제약(CON-007)**: 거래 순서는 `BUY -> SELL -> BUY -> SELL`만 유효하다.
3. **연속 매수 금지**: `last_action == BUY` 상태에서 추가 BUY 시도 시 `AlternationRuleViolationError` 또는 스킵 처리.
4. **보유 중 추가 매수 금지**: `is_bought == True`이면 항상 `should_buy=False`.

### 4.8 하위 호환성(기존 전략 1/2/3) 보장 노트

1. 기존 전략 1/2/3의 입력/출력 시그니처는 유지한다.
2. 기존 전략 1/2/3은 기존과 동일하게 당일 1매수/1매도 제한을 유지한다.
3. 기존 전략 1/2/3은 기존 손절 규칙(15:00/15:05)을 그대로 유지한다.
4. 전략 A/B 추가로 인해 기존 전략의 수익률 계산/매도 판정 로직이 변경되지 않도록 분리 구현한다.
5. `TradeSignal` 기본 필드는 하위 호환을 위해 기존 스키마를 유지하고, 신규 메타는 `meta` 확장 필드에만 추가한다.
...
sequenceDiagram
    ...
    alt 미등록 전략
        ...
        alt 입력 데이터 부족/시간대 오류
            ...
            SIM->>STR: evaluate(...)
            alt RSI 누락(전략3/A)
                STR-->>SIM: TradeSignal(NO_TRADE or SKIP, reason=RSI_MISSING)
            else 교대 규칙 위반(A/B)
...
| `RSIDataMissingError` | 전략3/A 매수 시점 RSI 없음/NaN | 해당 시점 매수 스킵 | `meta`에 기록 |
| `AlternationRuleViolationError` | A/B에서 `BUY->BUY` 시도 | 시도 거절 + 다음 캔들 진행 | 비치명(계속 진행) |
| `InsufficientSeedMoneyError` | `buy_quantity == 0` | 해당 시점 매수 포기 | 비치명(계속 진행) |
| `StrategyExecutionError` | 0나눗셈/비정상 상태 전이 | 해당 거래일 실패 | 시뮬레이션 엔진에서 처리 |

### 5.2 엣지 케이스 노트

1. **A/B 종가 미청산**: 이익보전 조건이 끝내 충족되지 않으면 당일 미청산 포지션이 남을 수 있으며, 상위 엔진 정책(이월 금지/강제청산)이 별도로 필요하다.
2. **RSI 단절 구간**: 전략 A는 RSI가 연속 NaN이면 진입 기회를 모두 놓칠 수 있으므로, 누락 원인을 `meta`로 기록한다.
3. **고변동 급락 후 급반등**: 전략 B에서 동일 캔들 내 경계값 충족이 연속 발생할 수 있으므로 캔들당 체결 횟수는 1회로 제한한다.
4. **최고수익률 0 근접**: 이익보전율 계산 시 분모 안정성(`epsilon`) 방어 로직 필요.
...
| 전략A | 09:05 | 일중 순회 시작 기준 시점 정합성 검증 |
| 전략B | 09:05 | 기준가 설정을 위해 필수 |
...
3. 손절 컷오프 판정은 KST `15:00` 기준으로 수행한다(전략1/2/3 전용).
...
3. 필요 시 `can_open_new_position` 오버라이드로 진입 제약 구현
4. 전략 고유 식별자(`name`)와 설명(`description`) 선언
5. `StrategyRegistry.register(new_strategy)`로 등록
6. 시뮬레이션 설정에서 전략 식별자만 주입(엔진 로직 수정 없음)

### 7.2 확장 제약사항

- 기존 전략 클래스(1/2/3) 코드 수정 금지
- `TradeSignal` 기본 출력 스키마 변경 금지(하위 호환 유지)
...
### 7.4 하위 호환성 체크포인트 (1/2/3)

| 체크 항목 | 보장 기준 |
|----------|-----------|
| 전략 식별자 | 기존 식별자 문자열 불변 |
| 손절 동작 | 기존 15:00/15:05 손절 기준 불변 |
| 일중 체결 횟수 | 기존 1매수/1매도 불변 |
| 결과 스키마 | 기존 필드 의미 불변 (`meta` 확장만 허용) |
| 레지스트리 조회 | 기존 `get(name)` 결과 타입/예외 불변 |
...
function should_sell_strategy1(candle, context):
    ...
    if context.is_trailing_started:
        ...
        preserve_ratio = (profit_rate / max(context.highest_profit_rate, epsilon)) * 100
...
function initialize_day_strategy2(candle_0905, seed_money):
    ...
    context.allow_same_day_stop_loss = true
    context.allow_multiple_round_trips = false
    return context

function should_buy_strategy2(candle, context, _):
    if context.trade_cycles >= 1 or candle.time <= "09:05":
        return false
...
```text
function initialize_day_strategyA(candle_0905, seed_money):
    context.allow_same_day_stop_loss = false
    context.allow_multiple_round_trips = true
    context.last_action = NONE
    return context

function can_open_new_position_strategyA(context):
    if context.is_bought:
        return false
...
function should_buy_strategyA(candle, context, rsi_value):
    if rsi_value is null or isNaN(rsi_value):
        record_meta("RSI_MISSING", candle.timestamp)
        return false

    if rsi_value <= 30 and can_open_new_position_strategyA(context):
        return true

    return false

function should_sell_strategyA(candle, context):
    return should_sell_strategy1(candle, context)

function should_stop_loss_strategyA(candle, context):
    return false
```
...
```text
function initialize_day_strategyB(candle_0905, seed_money):
    context.reference_price = candle_0905.close
    context.low_point = null
    context.allow_same_day_stop_loss = false
    context.allow_multiple_round_trips = true
...
function can_open_new_position_strategyB(context):
    if context.is_bought:
        return false
    if context.last_action == BUY:
        raise AlternationRuleViolationError("BUY_AFTER_BUY_NOT_ALLOWED")
...
function should_buy_strategyB(candle, context, _):
    ...
    if context.low_point is not null:
        context.low_point = min(context.low_point, candle.close)
        rebound_rate = ((candle.close - context.low_point) / context.low_point) * 100
        if rebound_rate >= 0.2 and can_open_new_position_strategyB(context):
            return true

    return false

function should_sell_strategyB(candle, context):
    return should_sell_strategy1(candle, context)

function should_stop_loss_strategyB(candle, context):
    return false
```
...
| FR-016 | 전략A: RSI<=30 매수 + 매도 trailing, 무손절, 교대 | 2.6, 3.2, 4.5, 4.7, 8.5 | `RSIOnlyTrailingStopStrategy` | RSI 진입 + 교대 불변식 + 손절 비활성 명세 |
| FR-017 | 전략B: 하락/반등 매수 + 매도 trailing, 무손절, 교대 | 2.7, 3.3, 4.6, 4.7, 8.6 | `BuyTrailingThenSellTrailingStrategy` | 기준가/전저점 진입 + 교대 불변식 + 손절 비활성 명세 |
| FR-006 | 신규 전략 확장성 | 2.8, 7 | `BaseStrategy`, `StrategyRegistry` | 추상 인터페이스 + 레지스트리 등록 절차 정의 |
| CON-006 | 전략A/B 당일 손절 미적용 | 2.6, 2.7, 4.7, 8.5, 8.6 | 전략 A/B 구현체 | `should_stop_loss=False` 강제 |
| CON-007 | 전략A/B 연속 매수 금지 및 교대 강제 | 2.2, 4.7, 5.1, 8.5, 8.6 | `BaseStrategy`, 전략 A/B 구현체 | `last_action` 상태 전이 + 위반 처리 |
| NFR-008 | 전략 확장 용이성 | 7.1~7.4 | `BaseStrategy`, `StrategyRegistry` | OCP 체크포인트 및 호환성 체크 정의 |
...
1. `BaseStrategy.evaluate`
   - 캔들 정렬 순회 및 상태 전이(`NONE->BUY->SELL`) 불변식 검증
   - `allow_same_day_stop_loss` 플래그별 분기(기존 전략/신규 전략) 검증

2. 기존 전략 1/2/3
   - 기존 경계값(1.0%, 80%, 0.2%, RSI 30, 09:05/15:00/15:05) 회귀 검증
   - 당일 1매수/1매도 제한 유지 검증

3. `RSIOnlyTrailingStopStrategy` (A)
   - RSI<=30 진입, RSI>30 비진입 검증
   - `BUY->BUY` 시도 차단(교대 위반) 검증
   - `should_stop_loss=False` 고정 검증

4. `BuyTrailingThenSellTrailingStrategy` (B)
   - 하락 1.0% 시작점, 전저점 갱신, 반등 0.2% 경계값 검증
   - 교대 규칙 및 보유 중 재매수 차단 검증
   - `should_stop_loss=False` 고정 검증

5. `StrategyRegistry`
   - 5전략 기본 등록/조회/목록 무결성 검증
   - 중복 등록 방지, 미등록 조회 예외 검증

### 10.2 통합 검증 포인트

- `SimulationEngine -> StrategyRegistry -> Strategy.evaluate` 경로의 전략 선택 정확성(1/2/3/A/B)
- 동일 입력 재실행 시 동일 결과 산출(결정성) 검증
- A/B 추가 후 기존 전략 1/2/3 결과 비변경 회귀 검증

### 10.3 검증 완료 기준

- FR-003~FR-006, FR-016, FR-017, CON-006, CON-007, NFR-008 추적 항목별 테스트 케이스 연결 100%
- 경계값(1.0%, 80%, 0.2%, RSI 30, 09:05/15:00/15:05) 모두 검증
- 제약 시나리오(무손절, 교대 위반, RSI 누락, 필수 시점 누락) 재현 가능
